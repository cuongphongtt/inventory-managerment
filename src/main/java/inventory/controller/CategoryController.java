...
public class CategoryController {
	...
	public String showCategoryList(Model model,HttpSession session,@ModelAttribute("searchForm") Category category,@PathVariable int page) {
		Paging paging = new Paging(3);
		paging.setIndexPages(page);//Dựa vào trang hiện tại để tính ra offset bắt đầu từ bao nhiêu
		List<Category> categories = productService.getAllCategory(category,paging);
		if(session.getAttribute(Constant.MSG_SUCCESS)!=null) {
			model.addAttribute(Constant.MSG_SUCCESS,session.getAttribute(Constant.MSG_SUCCESS));
			session.removeAttribute(Constant.MSG_SUCCESS);
		}else if(session.getAttribute(Constant.MSG_ERROR)!=null) {
	...
	public String delete(Model model , @PathVariable("id") int id,HttpSession session) {
		...
		if(category!=null) {
			try {
				...
				e.printStackTrace();
				log.error(e.getMessage());
				session.setAttribute(Constant.MSG_ERROR, "Delete Has Errors");
			}
		}
		return "redirect:/category/list";
	}
